document.getElementById('msvm').onclick = function(){
    var [modal, modal_body, modal_submit] = createModal('msvm-modal', 'Multi SVM')

    var form  = createElement('form')

    var alpha_input = createElement('input', {type: 'number', value: 0.001, placeholder: 'Alpha...', className: 'form-control'})
    form = createFormGroup(form, alpha_input, 'Alpha')
    
    var iterations_input = createElement('input', {type: 'number', value: 5, placeholder: 'İterations...', className: 'form-control'})
    form = createFormGroup(form, iterations_input, 'İterations')
    
    var c_input = createElement('input', {type: 'number', value: 5.0, placeholder: 'C...', className: 'form-control'})
    form = createFormGroup(form, c_input, 'C')
    
    //var kernel_input = createElement('input', {type: 'text', value: 'gaussian', placeholder: 'Kernel...', className: 'form-control'})
    //form = createFormGroup(form, c_input, 'Kernel')
    ...
    modal_submit.onclick = function(){
        ...
        var iterations = iterations_input.value
        var c = c_input.value
        //var kernel = kernel_input.value
        var predict = predict_select.value
        modal.remove()

        var cols = getSelectedCols()
        if(cols.includes(predict)){
        ...
        var train = df2matrix(X_train, cols)
        var test = df2matrix(X_test, cols)
        

        var msvmClassifier = new jssvm.MultiClassSvmClassifier({
            alpha: alpha,
        ...
        msvmClassifier.fit(train._data)
        
        var results = {target: X_test[predict].data.slice(), predicted: []}
        var result
        for(var i = 0; i < test._size[0]; i++){
            result = msvmClassifier.transform(test._data[i]);
